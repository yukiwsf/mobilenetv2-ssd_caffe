...
int main(int argc, char *argv[]) {
    /* init network */
    std::string modelFile = "../model/mbv2-ssd_1.prototxt";
    std::string weightFile = "../model/mbv2-ssd_1.caffemodel";
    Detector detector = Detector(modelFile, weightFile);

    /* read/save image path */        
    cv::String imagePath = "../data/bus.png";
    ...
    for(int i = 0; i < objects.size(); ++i) {
        ...
        char tmp[8];
        int ret = snprintf(tmp, sizeof(tmp) / sizeof(tmp[0]), "%.2f", objects[i].score);
        cv::String conf = tmp;
